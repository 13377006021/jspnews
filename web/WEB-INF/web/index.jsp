...
${pageContext.setAttribute("title","关注你所想")}
        ...
        <div class="content">
            <c:forEach items="${requestScope.map.focusNews}" var="foc" begin="0" end="0">
                <div class="firstImgNews" >
                    ...
                    <span class="abstract">
                    <a target="_blank" title="<c:out value="${m:getAbstract(foc.news.title,8)}"/>" href="/category/${m:getCategoryAliasById(requestScope.map.header.category,foc.news.category)}/${foc.news.id}.html" class="title">${m:getAbstract(foc.news.title,10)}</a>
                    <p style="font-size: 13px;">${m:getAbstract(foc.news.text,42)}</p>
                </span>
                    <div class="clear_float"></div>
                </div>
            </c:forEach>
