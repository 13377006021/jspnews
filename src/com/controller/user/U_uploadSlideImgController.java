...
@WebServlet(name = "U_uploadSlideImgController", urlPatterns = {"/user/U_uploadSlideImgController"})
@MultipartConfig
public class U_uploadSlideImgController extends HttpServlet {
    ...
    public void startUpload(Part part,HttpServletRequest request,HttpServletResponse response,String nowDate,boolean g) throws IOException {
        ...
        if(g){
            ...
            String s = null;
            Map<String, Serializable> map = new HashMap<String, java.io.Serializable>();
            if(imgPath!=null){
                map.put("code",200);
                map.put("msg","图片更新成功!");
                map.put("url",imgPath);
